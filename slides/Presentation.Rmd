...
- Hypothesis:
    - The number of deaths would *increase* with an increase in maximum sustained windspeed and property damage, as intuitively it would make sense for more destructive hurricanes to also be more deadly.
...
- Dataset "hurricNamed" from the "DAAG" R package 
    - Data on 94 named hurricanes that made landfall in the US mainland from 1950 through 2012. 
    - Includes: number of deaths, name of the hurricane, the year of the hurricane, the damage caused by the hurricane, etc.
...
**Explanatory Variables**
  ...
  - `LF.WindsMPH`: maximum sustained windspeed for each hurricane in miles per hour
  - `LF.PressureMB`: atmospheric pressure at landfall in millibars
  - `LF.times`: number of times the hurricane made landfall 
  - `BaseDam2014`: property damage caused by the hurricane in millions of 2014 US dollars
  - `NDAM2014`: amount of damage the hurricane caused had it appeared in 2014 (no units given)
...
- Found that the best model predicting `log_deaths` was again one that used only `log_NDAM2014`. 
    - Multiple $R^2$ = 48.07%, which was decent
...
ggplot(JHM, aes(x = residuals, y = fitted)) +
  geom_point() + 
  labs(title = "Residuals vs Fitted Values for JHM", xlab = "Residuals", 
       ylab = "Fitted")
...
- GAM using a b-spline on log `NDAM2014`, and a cubic polynomial for `LF.PressureMB` proved to be best predictor of log number of deaths from a US hurricane that made landfall between 1950-2012 
    - Lowest AIC = 290.88
    - Better graphical representation and ability to fit local trends
